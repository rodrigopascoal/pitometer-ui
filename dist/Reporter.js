...
class Reporter {
     ...
     * @param runResults
     */
    generateTimeseriesForReport(runResults) {
        ...
        for (var resultIx = 0; resultIx < runResults.length; resultIx++) {
            ...
            for (var indResIx = 0; indResIx < runResult.indicatorResults.length; indResIx++) {
                var indResult = runResult.indicatorResults[indResIx];
                ...
                if (indResult.individualResults) {
                    ...
                    for (var indivResIx = 0; indivResIx < indResult.individualResults.length; indivResIx++) {
                        var individualResult = indResult.individualResults[indivResIx];
                        ...
                        var reportSeries = reportSeriesMap.get(seriesId);
                        if (!reportSeries) {
                            reportSeries = new Map();
                    ...
                    var reportSeries = reportSeriesMap.get(indResult.id);
                    if (!reportSeries)
                        reportSeries = { "name": indResult.id, "data": [] };
     ...
     */
    generateHtmlReport(mainReport, seriesTemplate, seriesPlaceholder, timeseriesData) {
        var resultFile = mainReport;
